...
| AG-UI protocol bridge                                                  | ✅      | ❌     | ❌          | Agentsy includes a protocol-oriented bridge surface via `@agentsy/runtime/ag-ui` that is separate from any one app framework.                                                            |
...
- **Protocol-aware projection layers.** `@agentsy/runtime/ag-ui` and `@agentsy/ui` make state and protocol projection explicit rather than burying them inside one client abstraction.
