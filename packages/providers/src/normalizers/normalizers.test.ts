...
describe(normalizeOpenAIChatChunk, () => {
      ...
      choices: [
        {
          delta: {
            tool_calls: [
              { function: { arguments: '{"city"', name: null }, index: 0 },
            ],
          },
          finish_reason: null,
          index: 0,
  ...
  it("never throws on adversarial input", () => {
    expect(() =>
      normalizeOpenAIChatChunk({ choices: "not-an-array" })
    ).not.toThrow();
...
describe("openai-compatible normalizer", () => {
  it("exposes expected provider registry", () => {
    expect(OPENAI_COMPATIBLE_NORMALIZER_PROVIDERS).toStrictEqual([
      "openai",
      "kimi",
      "qwen",
      "llama",
...
describe(normalizeOpenAIResponseEvent, () => {
  ...
  it("maps response.output_item.added for function_call to nativeToolCallDeltas", () => {
    ...
    expect(result?.chunk.nativeToolCallDeltas?.[0]?.name).toBe("get_weather");
    expect(
      result?.chunk.nativeToolCallDeltas?.[0]?.argumentsDelta
    ).toBeUndefined();
  });
  ...
  it("returns null for unknown event types", () => {
    expect(
      normalizeOpenAIResponseEvent({ type: "response.created" })
    ).toBeNull();
    expect(
      normalizeOpenAIResponseEvent({ type: "response.in_progress" })
    ).toBeNull();
    expect(
      normalizeOpenAIResponseEvent({ type: "response.output_item.done" })
    ).toBeNull();
    expect(
      normalizeOpenAIResponseEvent({ type: "something.unknown" })
    ).toBeNull();
  });
...
describe(normalizeAnthropicEvent, () => {
  ...
  it("returns null for unknown/informational event types", () => {
    expect(
      normalizeAnthropicEvent({ index: 0, type: "content_block_stop" })
    ).toBeNull();
  ...
  it("never throws on adversarial input", () => {
    expect(() =>
      normalizeAnthropicEvent({ delta: null, type: "content_block_delta" })
    ).not.toThrow();
    expect(() =>
      normalizeAnthropicEvent({ message: null, type: "message_start" })
    ).not.toThrow();
...
describe(normalizeOllamaChatChunk, () => {
  ...
  it("never throws on adversarial input", () => {
    ...
    expect(() =>
      normalizeOllamaChatChunk({ message: { tool_calls: "bad" } })
    ).not.toThrow();
...
describe(normalizeGeminiChunk, () => {
  ...
  it("maps Gemini finishReason SAFETY to finishReason content-filter", () => {
    const result = normalizeGeminiChunk({
      candidates: [
        { content: { parts: [], role: "model" }, finishReason: "SAFETY" },
      ],
    });
    expect(result?.chunk.finishReason).toBe("content-filter");
  });
  ...
  it("never throws on adversarial input", () => {
    ...
    expect(() =>
      normalizeGeminiChunk({ candidates: [{ content: null }] })
    ).not.toThrow();
...
describe(normalizeMistralChunk, () => {
  it("maps delta.content to chunk.content (OpenAI-compatible)", () => {
    const result = normalizeMistralChunk({
      choices: [
        { delta: { content: "Bonjour" }, finish_reason: null, index: 0 },
      ],
      id: "cmpl-abc",
      model: "mistral-large",
      object: "chat.completion.chunk",
...
describe(normalizeCohereEvent, () => {
  ...
  it("maps tool-call-start to nativeToolCallDeltas with id+name", () => {
    ...
    expect(result?.chunk.nativeToolCallDeltas).toHaveLength(1);
    expect(result?.chunk.nativeToolCallDeltas?.[0]?.index).toBe(0);
    expect(result?.chunk.nativeToolCallDeltas?.[0]?.id).toBe(
      "get_weather_abc123"
    );
    expect(result?.chunk.nativeToolCallDeltas?.[0]?.name).toBe("get_weather");
  });
  ...
  it("returns null for informational event types", () => {
    expect(
      normalizeCohereEvent({ id: "abc", type: "message-start" })
    ).toBeNull();
    expect(
      normalizeCohereEvent({ index: 0, type: "content-start" })
    ).toBeNull();
    expect(normalizeCohereEvent({ index: 0, type: "content-end" })).toBeNull();
    expect(
      normalizeCohereEvent({ index: 0, type: "citation-start" })
    ).toBeNull();
  });
  ...
  it("never throws on adversarial input", () => {
    expect(() =>
      normalizeCohereEvent({ delta: null, type: "content-delta" })
    ).not.toThrow();
    expect(() =>
...
describe(normalizeBedrockConverseEvent, () => {
  ...
  it("sets done=true for all messageStop stopReason values", () => {
    expect(
      normalizeBedrockConverseEvent({ messageStop: { stopReason: "tool_use" } })
        ?.chunk.done
    ).toBeTruthy();
    expect(
  ...
  it("returns null for contentBlockStop and messageStart", () => {
    ...
    expect(
      normalizeBedrockConverseEvent({ messageStart: { role: "assistant" } })
    ).toBeNull();
  });
  ...
  it("never throws on adversarial input", () => {
    expect(() =>
      normalizeBedrockConverseEvent({ contentBlockDelta: null })
    ).not.toThrow();
    expect(() =>
      normalizeBedrockConverseEvent({ contentBlockDelta: { delta: null } })
    ).not.toThrow();
...
describe(normalizeHuggingFaceTGIChunk, () => {
  ...
  it("returns null for missing or non-object token field", () => {
    expect(
      normalizeHuggingFaceTGIChunk({ generated_text: null, index: 0 })
    ).toBeNull();
  ...
  it("never throws on adversarial input", () => {
    ...
    expect(() =>
      normalizeHuggingFaceTGIChunk({ token: { text: null } })
    ).not.toThrow();
