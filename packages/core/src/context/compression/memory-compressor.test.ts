...
describe(compressMemoryFile, () => {
  ...
  it("does not overwrite an existing backup on a second run", async () => {
    const dir = await mkdtemp(
      join(TEST_TMP_BASE, "agentsy-memory-nooverwrite-")
    );
    const filePath = join(dir, "CLAUDE.md");
    const backupPath = `${filePath}.original.md`;

    const original =
      "This is basically the very original content that should not be lost.";
    await writeFile(filePath, original, "utf-8");

    await compressMemoryFile(filePath, { backup: true });

    await expect(
      compressMemoryFile(filePath, { backup: true })
  ...
  it("refuses to compress sensitive paths", async () => {
    ...
    await expect(compressMemoryFile(filePath)).rejects.toThrow(
      "Refusing to compress sensitive path"
    );
  });
});
