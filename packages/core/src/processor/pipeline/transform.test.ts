...
async function collectStream(
  stream: ReadableStream<OutputPart>
): Promise<OutputPart[]> {
  const parts: OutputPart[] = [];
...
describe(createSmoothStream, () => {
  ...
  it("passes non-text parts through unchanged", async () => {
    ...
    const parts = await writeAndCollect(createSmoothStream({ chunkSize: 4 }), [
      part,
    ]);
  ...
  it("defaults to chunkSize 8", async () => {
    const parts = await writeAndCollect(createSmoothStream(), [
      { text: "x".repeat(16), type: "text" },
    ]);
    expect(parts).toHaveLength(2);
    expect(
      parts.every((p) => p.type === "text" && p.text.length === 8)
    ).toBeTruthy();
  });
...
describe(createToolCallFilter, () => {
  it("passes through only tool calls with matching names", async () => {
    const parts = await writeAndCollect(
      createToolCallFilter(["search", "fetch"]),
      [makeCall("search"), makeCall("delete"), makeCall("fetch")]
    );
    expect(parts).toHaveLength(2);
...
describe("LLMStreamProcessor.partsStream", () => {
  ...
  it("chains multiple transforms in order", async () => {
    const processor = new LLMStreamProcessor({
      transforms: [
        createThinkingFilter(),
        createSmoothStream({ chunkSize: 3 }),
      ],
    });
    const streamPromise = collectStream(processor.partsStream);
    // processComplete calls process() + flush() internally, ensuring the stream is closed.
