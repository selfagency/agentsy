...
function run(
  filter: ReturnType<typeof createXmlStreamFilter>,
  chunk: string
): string {
...
describe(createXmlStreamFilter, () => {
  ...
  it("still scrubs defaults when extraScrubTags are added", () => {
    const filter = createXmlStreamFilter({
      extraScrubTags: new Set(["custom"]),
    });
    const out = run(
      filter,
      "<context>gone</context><custom>also gone</custom>kept"
    );
  ...
  it("passes through CDATA adjacent to scrubbed content", () => {
    ...
    const out = run(
      filter,
      "before<context>gone</context><![CDATA[kept]]>after"
    );
  ...
  it("suppresses XML comments inside scrubbed tags", () => {
    ...
    const out = run(
      filter,
      "<context><!-- hidden comment -->secret</context>visible"
    );
    expect(out).toContain("visible");
  ...
  it("suppresses content that exceeds maxXmlNestingDepth", () => {
    ...
    const out = filter.write(
      "<outer><inner><deep>hidden</deep></inner></outer>"
    );
    filter.end();
