...
function resolveScrubTagSet(
  options: CreateXmlStreamFilterOptions
): Set<string> {
  if (options.overrideScrubTags) {
    ...
    const missingPrivacyTags = [...PRIVACY_TAG_NAMES].filter(
      (tag) => !override.has(tag)
    );
    if (missingPrivacyTags.length > 0 && enforcePrivacyTags) {
      ...
      options.onWarning?.(
        "Privacy-sensitive tags omitted from scrub override; enforcing defaults.",
        {
          missingPrivacyTags,
        }
      );
...
export function createXmlStreamFilter(
  options: CreateXmlStreamFilterOptions = {}
): XmlStreamFilter {
  const scrubTagNames = resolveScrubTagSet(options);
  const parser = new Saxophone();
  const maxXmlNestingDepth =
    options.maxXmlNestingDepth ?? DEFAULT_MAX_XML_NESTING_DEPTH;
  ...
  parser.on("tagopen", (tag: SaxophoneTag) => {
    if (!tag.isSelfClosing) {
      parseDepth++;
      if (
        maxXmlNestingDepth > 0 &&
        overflowStartDepth === null &&
        parseDepth > maxXmlNestingDepth
      ) {
        overflowStartDepth = parseDepth;
        options.onWarning?.(
          "XML nesting depth exceeded maxXmlNestingDepth; suppressing nested segment.",
          {
            depth: parseDepth,
            maxXmlNestingDepth,
          }
