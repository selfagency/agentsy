...
export function runSandboxDiagnosticsCommand(argv: readonly string[], io: CliIO = defaultIo): number {
  ...
  stdout('');
  stdout(`Default trigger mode:    ${result.defaultTrigger.mode} (${result.defaultTrigger.reason})`);
  stdout(`Untrusted trigger mode:  ${result.untrustedTrigger.mode} (${result.untrustedTrigger.reason})`);
  return 0;
}
