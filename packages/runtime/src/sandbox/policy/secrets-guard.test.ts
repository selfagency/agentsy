...
describe(guardSecrets, () => {
  ...
  it("detects gho_ pattern (GitHub token)", () => {
    const result = guardSecrets(
      "auth: gho_abcdefghijklmnopqrstuvwxyz0123456789aa"
    );
    expect(result.safe).toBeFalsy();
...
describe(assertSecretsGuard, () => {
  ...
  it("throws when secrets detected (default strict mode)", () => {
    expect(() =>
      assertSecretsGuard("key: sk-abcdefghijklmnopqrstu1234567890AB")
    ).toThrow(/Secrets detected/);
  });

  it("does not throw in non-strict mode, returns redacted string", () => {
    const out = assertSecretsGuard("sk-abcdefghijklmnopqrstu1234567890AB", {
      strict: false,
    });
