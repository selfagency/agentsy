...
describe('VirtualSandbox', () => {
  ...
  it('should execute code and return ok status', async () => {
    ...
    const messageCall = mockWorkerInstance.on.mock.calls.find((c: any[]) => c[0] === 'message');
  ...
  it('should handle worker error', async () => {
    ...
    const errorCall = mockWorkerInstance.on.mock.calls.find((c: any[]) => c[0] === 'error');
  ...
  it('should handle non-zero exit code', async () => {
    ...
    const exitCall = mockWorkerInstance.on.mock.calls.find((c: any[]) => c[0] === 'exit');
