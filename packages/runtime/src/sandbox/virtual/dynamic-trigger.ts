...
export function decideSandboxTrigger(ctx: SandboxTriggerContext): SandboxTriggerDecision {
  if (ctx.forceMode !== undefined) {
    return { mode: ctx.forceMode, reason: `forced: ${ctx.forceMode}` };
  }

  if (ctx.readOnly === true) {
    return { mode: 'none', reason: 'read-only operations do not require a sandbox' };
  }
