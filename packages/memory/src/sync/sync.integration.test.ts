...
describe("Phase 2 sync integration", () => {
  it("syncs local and remote state end to end", async () => {
    const remoteRecords: SyncRecord[] = [
      createRecord("remote-1", "Remote note", "2026-05-15T00:00:00.000Z"),
    ];
    ...
    const result = await manager.sync({
      ...
      records: [
        createRecord("local-1", "Local note", "2026-05-15T01:00:00.000Z"),
      ],
    });
  ...
  it("persists unresolved conflicts for manual resolution workflows", async () => {
    ...
    const result = await manager.sync({
      cursor: "local-cursor-1",
      records: [
        createRecord("shared-1", "Local edit", "2026-05-15T01:00:00.000Z"),
      ],
    });
