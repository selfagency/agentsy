...
function chooseRecord(
  ...
  return {
    ...preferred,
    ...(preferred.metadata === undefined
      ? {}
      : { metadata: preferred.metadata }),
    ...(preferred.relationships === undefined
      ? {}
      : { relationships: [...preferred.relationships] }),
  };
}

function mergeWikiConflict(
  ...
  const metadata =
    precedence.metadata === "local" ? local.metadata : remote.metadata;
  const relationships =
    precedence.relationships === "local"
      ? [...(local.relationships ?? [])]
      : [...(remote.relationships ?? [])];
  const vectorFingerprint =
    precedence.vectorFingerprint === "local"
      ? local.vectorFingerprint
      : remote.vectorFingerprint;
...
export function collectConflicts(
  ...
  const remoteById = new Map(
    remoteBatch.map((record) => [createRecordKey(record), record])
  );
