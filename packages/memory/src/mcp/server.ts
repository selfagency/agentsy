...
import { createMcpServer, type McpServer, type McpServerOptions } from './protocol.js';
...
export async function createMemoryMCPServer(
  ...
  const serverOptions: McpServerOptions = {
    ...
    tools: Object.fromEntries(
      Object.entries(definitions).map(([name, def]) => [name, { definition: def, handler: handlers[name]! }])
    )
  };
  ...
  async function start(): Promise<void> {
    if (transport === 'stdio') {
      ...
      readlineInterface = readline.createInterface({ input: process.stdin, terminal: false });
      ...
      readlineInterface.on('line', async (line: string) => {
        ...
        try {
          ...
          const result = await server.handleMessage(msg);
