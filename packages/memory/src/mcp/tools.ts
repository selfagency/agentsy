import type {
  MemoryEngine,
  MemoryEngineIngestOptions,
  MemoryEngineRecallOptions,
} from "../cognitive/memory-engine.js";
...
function buildIngestOptions(
  ...
  if (
    ...
    kindValues.includes(args.kind as (typeof kindValues)[number])
  ) {
...
function buildRecallOptions(
  ...
  if (
    typeof args.writeHeap === "string" &&
    heapValues.includes(args.writeHeap as (typeof heapValues)[number])
  ) {
...
export function createMemoryMcpTools(engine: MemoryEngine): MemoryMcpToolSet {
  ...
  const recallHandler: McpToolHandler = async (args) => {
    ...
    return {
      content: [
        { type: "text", text: `Found ${totalItems}:\n${lines.join("\n")}` },
      ],
    };
  };
  ...
  handlers.memory_stats = async (_args) => {
    ...
    for (const [tierName, tierStat] of Object.entries(stats.tierStats)) {
      lines.push(
        `  ${tierName}: ${tierStat.items} items, ${tierStat.usedTokens}/${tierStat.maxTokens} tok`,
      );
    }
  ...
  handlers.memory_list = async (args) => {
    ...
    const lines = result.items.map(
      (item) =>
        `  ${item.id} (imp: ${item.importance.toFixed(2)}): ${item.content.slice(0, 120)}`,
    );
    return {
      content: [
        {
