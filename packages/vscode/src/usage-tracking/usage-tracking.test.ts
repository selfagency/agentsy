...
function makeDataSource(
  quota: UsageQuota = makeQuota()
): IQuotaDataSource & { quota: UsageQuota } {
  return {
    dispose: vi.fn(),
...
describe(formatQuotaText, () => {
  it("formats token quota", () => {
    const text = formatQuotaText(
      makeQuota({ percentUsed: 0.2, total: 5000, unit: "tokens", used: 1000 })
    );
  ...
  it("rounds percentage", () => {
    const text = formatQuotaText(
      makeQuota({ percentUsed: 1 / 3, total: 3, unit: "credits", used: 1 })
    );
...
describe("Quota adapter utilities", () => {
  ...
  it("pickActiveQuotaWindow honors preferred-order strategy", () => {
    ...
    const active = pickActiveQuotaWindow(windows, "preferred-order", [
      "hourly",
      "monthly",
    ]);
