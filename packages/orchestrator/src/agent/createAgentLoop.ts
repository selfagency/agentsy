...
function getSortedEntries(value: object): [string, unknown][] {
  return Object.entries(value).toSorted(([leftKey], [rightKey]) =>
...
function toolCallsEqual(
  ...
  const idsMatch =
    a.id !== undefined && b.id !== undefined ? a.id === b.id : true;
  return (
    a.name === b.name && idsMatch && parametersEqual(a.parameters, b.parameters)
  );
}

function mergeStepCallbacks(
  base: AgentLoopOptions,
  overrides: AgentLoopStepOverrides,
  merged: AgentLoopOptions
): void {
  assignCallbackIfDefined(
    merged,
    "beforeStep",
    mergeCallbacks(base.beforeStep, overrides.beforeStep)
  );
  assignCallbackIfDefined(
    merged,
    "onStep",
    mergeCallbacks(base.onStep, overrides.onStep)
  );
  assignCallbackIfDefined(
    merged,
    "afterStep",
    mergeCallbacks(base.afterStep, overrides.afterStep)
  );
}
...
function mergeToolAndFinalCallbacks(
  ...
  const beforeToolCall = mergeCallbacks(
    base.beforeToolCall,
    overrides.beforeToolCall
  );
  const afterToolCall = mergeCallbacks(
    base.afterToolCall,
    overrides.afterToolCall
  );
...
function deriveDeniedToolCalls(
  ...
  return toolCalls.filter(
    (toolCall) =>
      !approvedToolCalls.some((candidate) =>
        toolCallsEqual(candidate, toolCall)
      )
  );
}
...
function normalizeApprovalResult(
  ...
  if (result.deniedToolCalls && result.decision !== "allow") {
    return {
      approvedToolCalls: deriveDeniedToolCalls(
        toolCalls,
        result.deniedToolCalls
      ),
      deniedToolCalls: [...result.deniedToolCalls],
...
function mergeStepOptions(
  ...
  const { approveToolCalls: baseApproveToolCalls, ...baseWithoutApprove } =
    base;
  const {
    approveToolCalls: overrideApproveToolCalls,
  ...
  const resolvedApproveToolCalls = resolveApproveToolCalls(
    baseApproveToolCalls,
    overrideApproveToolCalls
  );
...
async function processSingleStep(
  ...
  const {
    ...
    parts: streamedParts,
    isAborted,
  } = await executeStep(options, currentMessages, abortController);
  ...
  const finalOutput: ProcessedOutput = {
    ...
    ...(accumulatedMessage.usage === undefined
      ? {}
      : { usage: accumulatedMessage.usage }),
  };
  ...
  if (
    stopConditions.some((condition) => condition(state)) ||
  ...
  const { approvedToolCalls, deniedToolCalls } = await resolveToolApproval(
    options,
    toolContext
  );
  ...
  const toolResultMessages =
    await options.buildToolResultMessages(approvedToolCalls);
  await options.afterToolCall?.({
    ...toolContext,
    approvedToolCalls,
    deniedToolCalls,
  ...
  const newMessages = [
    ...currentMessages,
    { content: finalOutput.content, role: "assistant" },
    ...toolResultMessages,
  ];

  const nextMessages =
    options.maxConversationMessages &&
    newMessages.length > options.maxConversationMessages
 ...
 * @internal
 */
function updateConsecutiveIdenticalCalls(
  state: AgentLoopState,
  ...
  if (state.steps.length >= 2 && stepResult.toolCalls.length > 0) {
    ...
    if (prevStep && prevStep.toolCalls.length > 0) {
      ...
      if (
        prev.name === curr.name &&
        parametersEqual(prev.parameters, curr.parameters)
      ) {
...
export function createAgentLoop(options: AgentLoopOptions): AgentLoopHandle {
  ...
  async function executeLoopIteration(
    ...
    const stepOverrides =
      (await options.prepareStep?.(loopContext)) ?? undefined;
  ...
  async function* run(initialMessages: unknown[]): AsyncGenerator<OutputPart> {
    ...
    try {
      while (!aborted && state.steps.length < maxSteps) {
        const iteration = await executeLoopIteration(
          runId,
          threadId,
          state,
          currentMessages
      ...
      const finishedOutcomeType =
        abortReason === "abort" ? "interrupt" : abortReason;
