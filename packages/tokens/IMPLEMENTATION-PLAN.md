...
export function validateCompression(
  ...
  if (originalPaths.size !== compressedPaths.size) {
    ...
    result.addWarning(
      `Path mismatch: lost=${lost.join(", ")}, added=${added.join(", ")}`
    );
  }
  ...
  if (diff > 0.15) {
    result.addWarning(
      `Bullet count changed too much: ${originalBullets} → ${compressedBullets}`
    );
  }
  ...
  if (originalInline.size !== compressedInline.size) {
    const lost = [...originalInline].filter(
      (code) => !compressedInline.has(code)
    );
...
export class MemoryLivenessPubSub {
  ...
  subscribe(
    stage: CompactStage,
    callback: (event: MemoryLivenessEvent) => void
  ): void;
