...
- **REQ-032**: Skills discovery must follow the agentskills.io open standard (SKILL.md format with YAML frontmatter name+description, progressive 3-stage disclosure: discover → activate → execute). Discovery roots in priority order: `<project>/.agents/`, `~/.agents/`, `~/.config/agentsy/skills/`, bundled package skills.
- **REQ-033**: Instructions discovery must walk standard instruction file locations at loop startup: `<project-root>/AGENTS.md`, `<project-root>/CLAUDE.md`, `<project-root>/.github/copilot-instructions.md`, `<project-root>/.cursor/rules/`, `~/.agentsy/instructions.md`. All discovered instruction content is always-injected into the prompt stack via `@agentsy/prompts` before each turn. Discovery and injection must be deterministic, budget-aware, and scoped.
- **REQ-034**: All memory layers (episodic, semantic, procedural, session, wiki) must be wired as first-class pre-turn and post-turn hooks in the agentic loop. Pre-turn: retrieve → pack → inject via `@agentsy/prompts`. Post-turn: capture relevant observations for each memory class. The runtime hook pipeline is the single source of truth for memory injection; memory must never be injected by ad-hoc caller code.
- **REQ-035**: A named hook registry must exist in `@agentsy/runtime` (or `@agentsy/orchestrator`) allowing first-party and user-defined hooks to be registered by name, ordered by priority, composed into the canonical `AgentLoopOptions` callbacks at loop construction. Hooks must be: typed by lifecycle event, capable of reading/mutating context, independently testable, and disableable per-session.
- **REQ-036**: Built-in agents must be defined as first-class `AgentDefinition` objects in `@agentsy/plugins/src/agents/`: at minimum `default`, `research`, `code`, `plan`, and `superagent` modes. Each agent defines: system prompt template, allowed tools, memory scope, orchestration mode, default model preferences, and hook set.
- **REQ-037**: Users must be able to define custom agents via `AGENT.md` files (YAML frontmatter + markdown instructions, following agentskills.io conventions) placed in `<project>/.agents/` or `~/.agents/`. The loader must parse and register them identically to built-in agents, subject to capability policy gating.
- **REQ-038**: An agent mode picker must be implemented as an Ink chooser component in `@agentsy/renderers/src/ink/components/agent-picker/` and wired into the CLI chat startup and `/agent` slash command flows. It must list built-in agents and user-defined custom agents with their descriptions, model preferences, and tool access summary.
- **REQ-039**: The plugin architecture must implement the agentskills.io open standard SKILL.md format as its primary first-party and third-party extension format. This provides Claude-Code-compatible plugin installation with zero translation layer, since Claude Code, GitHub Copilot, Cursor, and Gemini CLI all implement the same spec.
- **REQ-040**: Pre-turn, post-turn, pre-compact, pre-tool-call, post-tool-call, on-session-create, on-session-end, on-error | Hook taxonomy: 8 hook types with deterministic execution lifecycle | Phase 4 | TASK-HOOK-001-004
- **REQ-041**: PreCompact | Auto-compaction runtime primitive calls PreCompact hook before compact cycle | Phase 5 | TASK-MEM-010-012
- **REQ-042**: --plan | Plan mode flag configures session for no-tool-execution plan generation | Phase 4 | TASK-PLAN-001-003
- **REQ-043**: Plugin security | All plugin context-injection points must be audited before Phase 4 launch | Phase 4 | TASK-PLUGIN-020-022
- **REQ-044**: SKILL.md | Plugin-published SKILL.md files are discoverable via plugin manifest metadata | Phase 4 | TASK-SKILL-015-017
...
| `packages/llm-gateway`   | Phase 3.5                                              |
| `packages/models`        | Phase 3                                                |
| `packages/observability` | Phases 5, 9                                            |
| `packages/orchestrator`  | Phases 4, 4.5 (hook registry, builtin hooks, createAgentSession) |
| `packages/plugins`       | Phase 4 (skills discovery, instructions discovery, agent registry, built-in agents, custom user agents) |
| `packages/prompts`       | Phase 4 (instructions layer, skills layer types, instruction composer) |
| `packages/providers`     | Phases 2, 3                                            |
| `packages/renderers`     | Phases 2, 5                                            |
| `packages/retrieval`     | Phases 8, 10                                           |
| `packages/runtime`       | Phases 2, 4, 5, 6, 7, 9 (memory hook wiring: pre-turn, post-turn, wiki) |
...
| TASK-004 | Freeze a dogfood release contract document at `docs/developers/integration-copilot.md` + `docs/developer-guide.md` describing “CLI is primary integration surface”.                                                       | ✅        | 2026-05-24 |
| TASK-005 | Add milestone tracker section in `README.md` linking this plan as canonical execution order for production CLI delivery.                                                                                                  | ✅        | 2026-05-24 |
| TASK-006 | Add explicit CLI-first release checklist template in `packages/cli/README.md` for all subsequent phases.                                                                                                                  | ✅        | 2026-05-24 |
...
### Implementation Phase 3.5 — LLM Gateway scaffold and CLI provider routing integration

- GOAL-003.5: Scaffold `@agentsy/llm-gateway`, implement foundation (Phase 1 of LB plan), and wire `LLMGatewayClient` as the default provider client in the CLI.

| Task        | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Completed | Date |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- | ---- |
| TASK-LB-001 | Create `packages/llm-gateway/` package scaffold: `package.json` (`@agentsy/llm-gateway`), `tsconfig.json`, `tsup.config.ts`, `vitest.config.ts`. Add to monorepo turbo pipeline.                                                                                                                                                                                                                                                                                                                                      | ✅        | 2026-05-24 |
| TASK-LB-002 | Define `ProviderProfileConfigSchema` (Zod) and `ProviderProfile` code interface in `packages/llm-gateway/src/profiles/types.ts`. Types must be consistent with `NormalizerProvider` from `packages/providers/src/pipeline/create-pipeline.ts` and `ProviderCapabilities` from `packages/types/src/providers.ts`.                                                                                                                                                                                                        | ✅        | 2026-05-24 |
| TASK-LB-003 | Implement `fromConfig()` converter (`ProviderProfileConfig → ProviderProfile`) in `packages/llm-gateway/src/profiles/from-config.ts`, `genericHeaderParser()`, `genericErrorClassifier()`, and `genericProbe()`.                                                                                                                                                                                                                                                                                                        | ✅        | 2026-05-24 |
| TASK-LB-004 | Implement `ProfileRegistry` (`register`, `get`, `detectFromUrl`, `detectFromHeaders`) in `packages/llm-gateway/src/profiles/registry.ts`.                                                                                                                                                                                                                                                                                                                                                                               | ✅        | 2026-05-24 |
| TASK-LB-005 | Implement built-in provider profile configs in `packages/llm-gateway/src/profiles/builtins/`: `generic-openai.ts` (covers 15+ providers), plus Tier 0 (`ollama-local.ts`, `zai.ts`), Tier 1 (`openai.ts`, `anthropic.ts`, `gemini.ts`, `bedrock.ts`, `mistral.ts`, `deepseek.ts`, `xai.ts`, `perplexity.ts`, `ollama-cloud.ts`), and Tier 2 (`deepinfra.ts`).                                                                                                                                                           |           |      |
| TASK-LB-006 | Implement `ModelAliasMap` (`packages/llm-gateway/src/registry/model-alias.ts`) for logical model name → per-provider model name mapping. Seed with aliases for `gpt-4o`, `claude-opus-4`, `gemini-2.5-pro`, `llama-3.3-70b`. Consume `ModelSelector` from `packages/models/src/index.ts` to inform routing weights.                                                                                                                                                                                                     | ✅        | 2026-05-24 |
| TASK-LB-007 | Implement `ProviderRegistry` (`packages/llm-gateway/src/registry/index.ts`) that creates a `UniversalClient` instance (from `packages/providers/src/universal-client/client.ts`) per provider entry. Entries are sourced from `LLMGatewayConfig.providers` after profile resolution. Wire API key retrieval via `@agentsy/secrets` keychain — **never accept raw keys in config at runtime**.                                                                                                                         | ✅        | 2026-05-24 |
| TASK-LB-008 | Implement the `LLMGatewayClient` interface (`packages/llm-gateway/src/client.ts`) as a drop-in replacement for `UniversalClient`. Phase 3.5 implementation: single-provider passthrough with `getRoutingState()`, `markProviderUnhealthy()`, `markProviderHealthy()`, `getUsageSnapshot()`, and `shutdown()` stubs returning safe no-ops.                                                                                                                                                                             | ✅        | 2026-05-24 |
| TASK-LB-009 | Define `LLMGatewayConfigSchema` (Zod) in `packages/llm-gateway/src/config.ts` covering: `providers` (array of `ProviderEntrySchema`), `strategy` (`StrategyNameSchema` — default `adaptive`), `model` (alias string), `circuitBreaker` (`CircuitBreakerConfigSchema`), `retry` (`RetryConfigSchema`). Re-export `ProviderRetryPolicy` from `packages/types` or `packages/providers` rather than re-defining it.                                                                                                       | ✅        | 2026-05-24 |
| TASK-LB-010 | Update `packages/cli/src/providers/resolve-provider.ts` (TASK-008-A) to call `createLLMGatewayClient(config)` instead of `createUniversalClient()` directly. The `LLMGatewayConfig` is built from the CLI config's `providers` array (supporting multiple entries per model). Fall back to single-provider `LLMGatewayClient` when only one provider is configured — behaviour is identical.                                                                                                                        |           |      |
| TASK-LB-011 | Implement `CircuitBreaker` state machine (CLOSED/OPEN/HALF-OPEN), `HealthTracker` (error counting, latency tracking), and `LatencyTracker` (rolling percentile window) in `packages/llm-gateway/src/health/`. Wire health-aware provider filtering into the selection pipeline. Reuse `ProviderErrorCode.RateLimited` from `packages/vscode/src/types/errors.ts` and `STATUS_TO_ERROR_CODE` from `packages/vscode/src/error-handling/error-mapper.ts` for error classification.                                         |           |      |
| TASK-LB-012 | Implement `parseRateLimitHeaders()` for OpenAI, Anthropic, Meta, and generic profiles in `packages/llm-gateway/src/usage/header-parser.ts`. Create `__tests__/fixtures/header-samples.ts` with real header examples. Implement `LocalCounter` (RPM/TPM/concurrency window), `UsageTracker`, and `QuotaChecker` (pre-flight quota validation) in `packages/llm-gateway/src/usage/`.                                                                                                                                    |           |      |
| TASK-LB-013 | Implement all six routing strategies in `packages/llm-gateway/src/strategies/`: `RoundRobinStrategy`, `WeightedStrategy`, `LeastConnectionsStrategy`, `LatencyBasedStrategy`, `PriorityFallbackStrategy`, `CostBasedStrategy`, and `AdaptiveStrategy` (composite scorer). Strategy is selected via `LLMGatewayConfig.strategy`. Adaptive is the default.                                                                                                                                                              |           |      |
| TASK-LB-014 | Implement `retryWithFailover()` and `retryStreamWithFailover()` in `packages/llm-gateway/src/retry/`. Reuse `retry()` from `packages/core/src/retry/index.ts` for exponential backoff. Throw `AllProvidersExhaustedError` (with full diagnostic payload) when all providers fail. Wire into `LLMGatewayClient.complete()` and `LLMGatewayClient.stream()`.                                                                                                                                                          |           |      |
| TASK-LB-015 | Implement active usage probing (`packages/llm-gateway/src/probes/`) using the `usageProbe` config field from each `ProviderProfileConfig`. Probes call provider usage APIs (e.g. DeepInfra `GET /v1/me/rate_limit`) on a configurable interval and update `UsageTracker`. Adding a new provider's probe requires only a 3-line config addition — no TypeScript code.                                                                                                                                                    |           |      |
...
| TASK-LB-017 | Register `/lb status`, `/lb providers`, `/lb strategy <name>`, `/lb reset <providerId>` slash commands in `packages/plugins/src/slash-commands/registry.ts`. Handlers delegate to the session-scoped `LLMGatewayClient` instance.                                                                                                                                                                                                                                                                                       |           |      |
| TASK-LB-018 | Add `packages/llm-gateway/src/__tests__/` unit tests covering: config validation and `fromConfig()` conversion; `ProfileRegistry` lookup; model alias resolution; circuit breaker state transitions; header parsing for all built-in providers (using `header-samples.ts` fixtures); quota check pre-flight; each routing strategy with varied provider states; `retryWithFailover` exhaustion producing `AllProvidersExhaustedError`.                                                                                  |           |      |
| TASK-LB-019 | Add `packages/cli/src/e2e/llm-gateway.e2e.test.ts` covering: (a) primary provider returns 429 → automatic failover to secondary, transcript uninterrupted; (b) all providers exhausted → `AllProvidersExhaustedError` surfaces as user-facing message in CLI; (c) circuit breaker opens after 5 consecutive 500 errors → provider skipped; (d) `/lb strategy round-robin` changes strategy mid-session; (e) cost-based routing selects cheapest healthy provider. Use `packages/testing/src/msw/providers.ts` handlers. |           |      |
| TASK-LB-020 | Add `packages/llm-gateway/src/index.ts` public exports: `createLLMGatewayClient`, `LLMGatewayClient`, `LLMGatewayConfig`, `LLMGatewayConfigSchema`, `ProviderEntry`, `RoutingState`, `ProviderStatus`, `ProviderUsageSnapshot`, `StrategyName`, `AllProvidersExhaustedError`. Add TSDoc to all public symbols. Add `packages/llm-gateway/README.md` with migration guide (from direct `UniversalClient` to `LLMGatewayClient`), zero-config example, and multi-key failover configuration example.          |           |      |
...
| TASK-SIA-001 | Define `SkillManifest` Zod schema and `SkillDiscoverer` in `@agentsy/plugins/src/skills/` — walk roots, parse frontmatter, build metadata index (agentskills.io compliant).                                                 |           |      |
| TASK-SIA-002 | Implement `SkillActivator` + `createSkillsHook` in `@agentsy/plugins/src/skills/` — semantic matching, full-body loading, prepareStep callback for AgentLoopOptions.                                                         |           |      |
| TASK-SIA-003 | Define `InstructionFile` type and implement `InstructionsDiscoverer` in `@agentsy/plugins/src/instructions/` — walk standard instruction file locations, return InstructionFile[].                                            |           |      |
| TASK-SIA-004 | Export `createInstructionsHook(discoverer)` from `@agentsy/plugins/src/instructions/hook.ts` — beforeInit callback for AgentLoopOptions.                                                                                     |           |      |
| TASK-SIA-005 | Define `AgentDefinition` Zod schema and implement `AgentLoader` + `AgentRegistry` in `@agentsy/plugins/src/agents/` — discover AGENT.md files, merge with built-ins.                                                         |           |      |
| TASK-SIA-006 | Implement built-in agent definitions in `@agentsy/plugins/src/agents/builtins/` — default, research, code, plan, superagent. Each satisfies AgentDefinition.                                                                 |           |      |
| TASK-SIA-007 | Define `HookDefinition` type and implement `HookRegistry` in `@agentsy/orchestrator/src/hooks/` — register, unregister, enable, disable, getHandlersForEvent.                                                                |           |      |
| TASK-SIA-008 | Implement `compileHooks(registry, baseOptions)` in `@agentsy/orchestrator/src/hooks/compile.ts` — merges registry handlers into AgentLoopOptions callbacks.                                                                  |           |      |
| TASK-SIA-009 | Register builtin hooks in `@agentsy/orchestrator/src/hooks/builtins/` — memory pre-turn, memory post-turn, skills, instructions, budget, approval, observability.                                                            |           |      |
| TASK-SIA-010 | Implement `createAgentSession(agentDef, config)` in `@agentsy/orchestrator/src/session.ts` — loads agent, builds registry, compiles hooks, returns AgentLoopHandle.                                                          |           |      |
| TASK-SIA-011 | Implement memory hook wiring in `@agentsy/runtime/src/hooks/` — createMemoryPreTurnHook, createMemoryPostTurnHook, createWikiMemoryHook.                                                                                     |           |      |
| TASK-SIA-012 | Add `InstructionsLayer` + `SkillsLayer` segment types and `InstructionsComposer` to `@agentsy/prompts/src/layers/`.                                                                                                          |           |      |
| TASK-SIA-013 | Implement `AgentPickerComponent` in `@agentsy/renderers/src/ink/components/agent-picker/` — searchable list with provenance badges, arrow-key navigation.                                                                    |           |      |
| TASK-SIA-014 | Add `--agent <id>`, `/agent <id|?>`, `agentsy agents list/show`, `agentsy skills list/show` commands to `@agentsy/cli`.                                                                                                      |           |      |
| TASK-HOOK-001 | Define `HookType` enum (8 types: pre-turn, post-turn, pre-compact, pre-tool-call, post-tool-call, on-session-create, on-session-end, on-error) in `@agentsy/orchestrator/src/hooks/types.ts`.                                                             |           |      |
| TASK-HOOK-002 | Implement `HookRegistry` with type-safe dispatch in `@agentsy/orchestrator/src/hooks/registry.ts`.                                                                                                                                                           |           |      |
| TASK-HOOK-003 | Wire pre-turn hooks as read-only observers in `@agentsy/orchestrator/src/hooks/` — no side-effects, inform routing decisions only.                                                                                                                          |           |      |
| TASK-HOOK-004 | Wire post-turn hooks as side-effect phase in `@agentsy/orchestrator/src/hooks/` — memory persistence, logging execute here.                                                                                                                                |           |      |
| TASK-PLAN-001 | Add `--plan` mode flag to CLI `SessionOptions` in `@agentsy/cli/src/commands/chat.ts` and `@agentsy/types`.                                                                                                                                               |           |      |
| TASK-PLAN-002 | Wire `--plan` mode through orchestrator `createAgentSession` in `@agentsy/orchestrator/src/session.ts` — skip tool execution, return plan as structured output.                                                                                            |           |      |
| TASK-PLAN-003 | Short-circuit tool hooks in plan mode — all hooks run but tool-call hooks short-circuit immediately.                                                                                                                                                        |           |      |
| TASK-PLUGIN-020 | Audit all context-injection points in plugin loader (`@agentsy/plugins/src/loader.ts`) before Phase 4 launch.                                                                                                                                               |           |      |
| TASK-PLUGIN-021 | Add allowlist-based context field filtering in `@agentsy/plugins/src/security/` — only explicitly permitted fields pass through context injection.                                                                                                          |           |      |
| TASK-PLUGIN-022 | Document plugin security model in `packages/plugins/README.md` — context injection rules, allowlist configuration, and audit procedure.                                                                                                                     |           |      |
| TASK-SKILL-015 | Extend plugin manifest schema in `@agentsy/plugins/src/` to include optional `skills` field pointing to published SKILL.md files.                                                                                                                          |           |      |
| TASK-SKILL-016 | Implement SKILL.md discovery during plugin load — walk plugin directories, parse SKILL.md frontmatter, register with skill registry.                                                                                                                        |           |      |
| TASK-SKILL-017 | Surface discovered skills in agent definition resolution — `AgentDefinition` includes skills from activated plugins.                                                                                                                                        |           |      |
...
| TASK-MEM-010 | Implement `PreCompact` hook type in `@agentsy/runtime/src/hooks/` — called before each compact cycle, provides compaction budget context.                                                                                                  |           |      |
| TASK-MEM-011 | Register memory layer `PreCompact` handler in `@agentsy/memory/src/coordination/` — responds to compact signal, prunes low-value entries within budget.                                                                                    |           |      |
| TASK-MEM-012 | Add compaction budget awareness to handler — respects token/count ceilings set by `PreCompact` hook context.                                                                                                                               |           |      |
