...
**Evidence**: JuliusBrussee/caveman v1.7.0 (52.5k stars): the entire compression mechanism is a SKILL.md that instructs the model to produce compressed output. `caveman-shrink` MCP proxy compresses tool *descriptions* (input to model) — reducing prompt tokens, not output tokens.
...
**Decision**: `SlashCommandRegistry` in `@agentsy/agent` intercepts `/`-prefixed user messages *before* any model call. Matched commands execute directly, returning a synthetic assistant message. Unrecognized `/`-prefixed messages pass through unmodified.
