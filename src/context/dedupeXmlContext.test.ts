...
describe('dedupeXmlContextBlocksByTag', () => {
  it('handles nested tags of the same name and returns the outermost block', () => {
    ...
    expect(out).toEqual(['<b>other</b>', '<a>latest</a>']);
  });

  it('extracts full nested content for a deeply nested tag', () => {
    const blocks = ['<x><x><x>deep</x></x>outer</x>'];
    const out = dedupeXmlContextBlocksByTag(blocks);
    expect(out).toEqual(['<x><x><x>deep</x></x>outer</x>']);
  });

  it('handles triple-nested tags of the same name correctly', () => {
    const blocks = ['<ctx><ctx>middle<ctx>inner</ctx>middle</ctx>outer</ctx>'];
    const out = dedupeXmlContextBlocksByTag(blocks);
    // Should extract the entire outermost tag
    expect(out).toEqual(['<ctx><ctx>middle<ctx>inner</ctx>middle</ctx>outer</ctx>']);
  });
  ...
  it('handles tags with attributes and nested self-references', () => {
    const blocks = ['<myTag attr="val"><myTag>inner</myTag>outer</myTag>'];
    const out = dedupeXmlContextBlocksByTag(blocks);
    expect(out).toEqual(['<myTag attr="val"><myTag>inner</myTag>outer</myTag>']);
  });

  it('returns empty array when no valid tags are found', () => {
    const blocks = ['no tags here', 'plain text'];
    const out = dedupeXmlContextBlocksByTag(blocks);
    expect(out).toEqual([]);
  });

  it('handles self-closing tags correctly', () => {
    const blocks = ['<tag1/>', '<tag2>content</tag2>'];
    const out = dedupeXmlContextBlocksByTag(blocks);
    // Self-closing tags won't match the regex pattern, so only tag2 should appear
    expect(out).toEqual(['<tag2>content</tag2>']);
  });
