...
describe('extractXmlToolCalls', () => {
  it('extracts a single bare XML tool call with parameters', () => {
    ...
    const result = extractXmlToolCalls(xmlText, new Set(['search_files']));
  ...
  it('extracts multiple sequential tool calls in order', () => {
    ...
    const result = extractXmlToolCalls(xmlText, new Set(['read_file']));
  ...
  it('handles tool calls with no parameters', () => {
    ...
    const result = extractXmlToolCalls(xmlText, new Set(['list_tools']));
  ...
  it('extracts JSON-wrapped toolCall payload', () => {
    ...
    const result = extractXmlToolCalls(xmlText, new Set(['semantic_search']));
  ...
  it('returns empty array for malformed wrapped JSON', () => {
    const xmlText = '<toolCall>{"name":</toolCall>';
    const result = extractXmlToolCalls(xmlText, new Set(['semantic_search']));
    expect(result).toEqual([]);
  });
  ...
  it('returns empty array when known tools set is empty', () => {
    ...
    const result = extractXmlToolCalls(xmlText, new Set());
    expect(result).toEqual([]);
  });
  ...
  it('bare JSON fallback silently ignores unknown tool names', () => {
    const xmlText = '{"name":"unknown_tool","arguments":{"x":"y"}}';
    const result = extractXmlToolCalls(xmlText, new Set(['known_tool']));
    expect(result).toEqual([]);
  });
